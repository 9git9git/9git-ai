...
# 🔍 코딩 대회 정보를 Azure AI Search에서 검색 (RAG)
def search_notice(query: str) -> str:
    """
    사용자 질문(query)에 대해 Azure AI Search에서
    코딩 대회 정보(codingnoticeindex) 인덱스를 조회하고,
    ...
    index_name = os.getenv("AZURE_SEARCH_INDEX_NOTICE_CODING")        # 코딩 대회 정보를 담은 인덱스 
...
def upload_to_index(index_name: str, data: dict):
    """
    GPT가 생성한 응답 요약 데이터를 지정한 인덱스(codingtutorindex 등)에 업로드합니다.
