...
def handle_coding_tutor(user_input: str) -> str:
    ...
    upload_to_index(index_name, {
        "id": f"coding-{str(uuid4())}",
        "mode": "summary",
        "category": "coding",   # ✅ category 하드코딩 (다른 구조와 분리되었으므로 안전)
        "original": question,
        "summary": summary,
...
def load_combined_prompt() -> tuple[str, str]:
    """
    📂 prompts/coding.txt 파일을 불러와 system prompt와 user template를 분리
    - 구분자는 "🔸" 사용
    """
    prompt_path = os.path.join("prompts", "coding.txt")
