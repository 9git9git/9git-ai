...
from uuid import uuid4   # 각 문서에 고유한 ID를 부여하기 위해 사용


# 🔍 마라톤 대회 정보를 Azure AI Search에서 검색 (RAG)
def search_notice(query: str) -> str:
    """
    사용자의 검색어(query)를 기준으로 Azure AI Search에서
    마라톤 대회 정보(exercisenoticeindex) 인덱스를 검색하고,
    검색 결과 중 summary 필드를 문자열로 반환합니다.
    """
    ...
    index_name = os.getenv("AZURE_SEARCH_INDEX_NOTICE_EXERCISE")     # 마라톤 대회 정보를 담은 인덱스
...
def upload_to_index(index_name: str, data: dict):
    """
    GPT가 생성한 응답 요약 데이터를 지정한 인덱스(exercisetutorindex)에 업로드합니다.
